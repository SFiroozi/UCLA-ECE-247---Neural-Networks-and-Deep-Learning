...
def affine_forward(x, w, b):
  ...
  # ================================================================ #
  N  = x.shape[0]
  collection_of_ds = x.shape[1:]
  D = np.prod(collection_of_ds)
    
  forward_x_affine = np.reshape(x, (N,D))
  out = forward_x_affine.dot(w) + b
...
def affine_backward(dout, cache):
  ...
  derivitive_backwards = dout.dot(w.T)
  dx = np.reshape(derivitive_backwards,x.shape)
    
  backward_x_affine = np.reshape(x, (N,D))
  dw = (backward_x_affine.T).dot(dout)
...
def batchnorm_backward(dout, cache):
  ...
  dvar = (-0.5) * np.sum(power_denom * xc * dxn, axis =0)
  dmu = -1 * normalization_denom * np.sum(dxn, axis =0) - (2/N) * np.sum(xc, axis = 0) * dvar
